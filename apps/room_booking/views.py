...
class RoomListAPIView(APIView):
    def get(self, request):
        ...
        if start_date and end_date:
            ...
            # Filter rooms based on availability within the specified date range
            rooms = rooms.exclude(
...
class BookRoomAPIView(APIView):
    ...
    def post(self, request, room_id: UUID):
        ...
        if not request.user.is_authenticated:
            return Response({'message': 'User must be logged in to book a room'}, status=status.HTTP_401_UNAUTHORIZED)

        try:
            check_in_date = datetime.strptime(check_in_date, '%Y-%m-%d').date()
            check_out_date = datetime.strptime(check_out_date, '%Y-%m-%d').date()
        except ValueError:
            return Response({'message': 'Invalid date format. Please provide dates in the format YYYY-MM-DD'},
        ...
        if conflicting_reservations.exists():
            return Response({'message': 'Room is not available for booking'}, status=status.HTTP_400_BAD_REQUEST)
